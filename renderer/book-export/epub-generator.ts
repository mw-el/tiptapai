...
// Dynamic import for JSZip (Node module)
type JSZipModule = any;
let JSZipCtor: JSZipModule | null = null;
...
async function loadJSZip() {
  ...
  try {
    ...
    throw new Error(
      `JSZip nicht verfügbar: ${(error as Error).message}. Bitte 'npm install jszip' ausführen.`
    );
  }
}
...
function generateFrontmatterXhtml(book: BookIR): string {
  const lang = book.metadata.language || 'de';
  const blocks: string[] = [];
...
function renderBlockAsXhtml(block: Block): string {
  switch (block.type) {
    case 'paragraph':
...
${stepsHtml}
    ...
    case 'poem':
      return `    <div class="poem">${
        block.lines.map(l => `<p>${escapeXml(l) || '&#160;'}</p>`).join('')
      }</div>`;
    ...
    default:
      return `    <p>${escapeXml((block as any).text || '')}</p>`;
  }
}

// ============ OPF Manifest ============

function generateContentOpf(book: BookIR, coverFilename: string | null): string {
  ...
  for (let idx = 0; idx < book.chapters.length; idx++) {
    ...
    manifestItems.push(
      `<item id="chapter-${paddedIdx}" href="chapter-${paddedIdx}.xhtml" media-type="application/xhtml+xml"/>`
    );
  }
  ...
  if (coverFilename) {
    ...
    const mediaType = ext === 'png' ? 'image/png' :
                      ...
                      'image/jpeg';
    manifestItems.push(
      `<item id="cover-image" href="${escapeXml(coverFilename)}" media-type="${mediaType}" properties="cover-image"/>`
    );
  }
...
function generateNavXhtml(book: BookIR): string {
  const lang = book.metadata.language || 'de';
...
function generateTocNcx(book: BookIR): string {
  const uuid = generateUuid();
  const title = escapeXml(book.metadata.title);
...
function buildLicenseString(license?: License): string {
  if (!license) return '';
  if (license.type === 'CC') {
